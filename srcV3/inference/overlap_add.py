...
from srcV3.data.window_dataset import extract_window, load_cache, window_starts
from srcV3.models import WindowedSpeechModel
from srcV3.training.train_windows import parse_layers
...
def build_model_from_checkpoint(ckpt: dict, device: torch.device) -> WindowedSpeechModel:
    cfg = _default_config()
    ...
    model = WindowedSpeechModel(
        ...
        decoder_type=str(cfg.get("decoder_type", "direct_tcn")),
...
def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        axes[1].set_title("srcV3 overlap-add")
        fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run srcV3 window model with overlap-add.")
    parser.add_argument("--data-dir", default="Processed_Data_R2INR_Frontal_v2")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--output-dir", default="inference_srcV3_ola")
