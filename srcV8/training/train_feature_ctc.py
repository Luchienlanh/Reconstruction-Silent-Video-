...
from srcV8.data import AVFeatureCTCDataset, cer, collate_feature_ctc, greedy_decode, split_feature_files, wer
from srcV8.data.feature_dataset import build_vocab_from_feature_files
from srcV8.models import FeatureCTCModel
from srcV8.utils import batch_to_device, get_device, seed_everything, unwrap_model, write_json


def make_loader(args, files, vocab, batch_size: int, shuffle: bool) -> DataLoader:
    ds = AVFeatureCTCDataset(
        args.data_dir,
        vocab=vocab,
        files=files,
    ...
    return DataLoader(
        ...
        collate_fn=collate_feature_ctc,
    )


def ctc_loss(logits: torch.Tensor, batch: dict, model: FeatureCTCModel, criterion: torch.nn.CTCLoss) -> torch.Tensor:
...
def save_checkpoint(path, model, optimizer, epoch, best, args, vocab, input_dim: int):
    path = Path(path)
...
def run(args: argparse.Namespace) -> None:
    ...
    vocab = build_vocab_from_feature_files(train_files, text_unit=args.text_unit, min_freq=args.min_token_freq)
    ...
    print(f"[features] input_dim={input_dim} text_unit={args.text_unit} upsample={args.upsample_factor}")
    print(f"[model] srcV8 feature_ctc dim={args.dim} tcn={args.tcn_layers} transformer={args.transformer_layers}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_best:
            best = score
            save_checkpoint(output_dir / "best_model.pth", model, optimizer, epoch, best, args, vocab, input_dim)
        save_checkpoint(output_dir / "last_model.pth", model, optimizer, epoch, best, args, vocab, input_dim)
        row = {
            "epoch": epoch,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Vietnamese CTC head on cached AV-HuBERT visual features.")
    parser.add_argument("--data-dir", default="Processed_Data_AVHubertFeatures")
    parser.add_argument("--output-dir", default="checkpoints_srcV8_avhubert_ctc")
