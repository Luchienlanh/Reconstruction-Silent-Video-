"""
Simple full-mel training for silent-video-to-mel reconstruction.

This is the recommended next step after the one-sample overfit test:
train the whole dataset with one stable objective instead of phase curriculum.

Examples:
    python train_simple_mel.py --epochs 100 --data-dir Processed_Data_Mel_HiFiGAN_FullFrame
    python train_simple_mel.py --epochs 100 --encoder-type snn --batch-size 1 --max-frames 125
    python train_simple_mel.py --resume simple_mel_checkpoints/best_model.pth
"""
...
def unpack_batch(batch, device: torch.device):
    if len(batch) == 5:
        ...
        raise ValueError("This trainer expects VNLipDatasetV2 with landmarks and return_path=True.")
...
def run(args: argparse.Namespace) -> None:
    ...
    output_dir = resolve_path(args.output_dir) or PROJECT_ROOT / "simple_mel_checkpoints"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train full mel reconstruction without curriculum phases.")
    ...
    parser.add_argument("--limit", type=int, default=None, help="Use only first N samples for quick smoke tests.")
