...
from srcV2.models import ContextAlignLipToSpeechModel
...
def build_context_align_model(args, device: torch.device) -> torch.nn.Module:
...
def build_optimizer(model: torch.nn.Module, args) -> torch.optim.Optimizer:
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {
                "params": list(raw_model.upsample_prior.parameters())
                + list(raw_model.aligner.parameters())
                + list(raw_model.align_fusion.parameters()),
                "lr": args.lr * args.aligner_lr_scale,
            },
            {
                "params": list(raw_model.decoder.parameters()) + list(raw_model.detail_refine.parameters()),
                "lr": args.lr * args.decoder_lr_scale,
            },
            {
                "params": list(raw_model.mel_base_head.parameters())
...
def run(args) -> None:
    ...
    model = build_context_align_model(args, device)
    ...
    print(
        f"[model] ContextAlign dim={args.dim} spatial_tokens={args.spatial_tokens} "
        f"enc={args.encoder_layers} dec={args.decoder_layers} heads={args.heads} units={args.num_units} "
        f"align_window={args.align_window}"
    )
    print(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ContextAlign lip-to-speech model.")
    parser.add_argument("--data-dir", default="Processed_Data_R2INR")
    parser.add_argument("--output-dir", default="checkpoints_context_align")
    ...
    parser.add_argument("--aligner-lr-scale", type=float, default=2.0)
    ...
    parser.add_argument("--modality-dropout", type=float, default=0.05)
