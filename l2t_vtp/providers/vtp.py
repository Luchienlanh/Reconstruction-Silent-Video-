...
class VTPProvider:
    """Standalone adapter around prajwalkr/vtp for lip-to-text inference."""
    ...
    def _validate_paths(self) -> None:
        if not self.repo_dir.exists() or not (self.repo_dir / "models.py").exists():
            raise FileNotFoundError(f"VTP repo_dir does not look like prajwalkr/vtp: {self.repo_dir}")
        if not self.ckpt_path.exists():
            raise FileNotFoundError(f"VTP checkpoint does not exist: {self.ckpt_path}")
        if not self.cnn_ckpt_path.exists():
            raise FileNotFoundError(f"VTP feature extractor checkpoint does not exist: {self.cnn_ckpt_path}")

    def _build_model(self) -> torch.nn.Module:
        model = self._models_mod.builders[self.args.builder](
    ...
    def run(self, video_path: str | Path) -> VTPResult:
        ...
        with self._runtime_import_context():
            ...
            visual_features = torch.cat(features, dim=0) if features else torch.empty(0, int(self.args.feat_dim))
