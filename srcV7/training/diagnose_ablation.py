...
from srcV7.training.common import build_model, load_checkpoint, masked_stats, model_inputs
...
def apply_variant(batch: dict, variant: str) -> dict:
    ...
    if variant == "mismatch_sample" and out["video"].shape[0] >= 2:
...
def run(args) -> None:
    ...
    loader = DataLoader(ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=collate_r2cache)
    ...
    for key in (
        ...
        if hasattr(args, key):
            setattr(args, key, cfg.get(key, getattr(args, key)))
    ...
    criterion = MaskedMelLoss(
        ...
        lambda_delta=cfg.get("lambda_delta", 0.25),
        lambda_delta2=cfg.get("lambda_delta2", 0.05),
        lambda_energy=cfg.get("lambda_energy", 0.05),
    ).to(device)
